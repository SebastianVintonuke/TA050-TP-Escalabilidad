...
class QueryStateStorage:
    ...
    def check_integrity(self):
        ...
        for query_id, changes in query_changes.items():
            ...
            while i < len(changes) and changes[i][1].stat().st_mtime <= commit_ts:
                ...
                changes_to_apply = self.manager.deserialize_changes(changes[i][1].read_bytes())
                ...
                ## Create ack
                ack_file = self.finished / f"{query_id}_{packet_id}"
                ack_file.touch()
    ...
    def register_packet(self, query_id, packet_id):
        """
        ...
        if not file.exists(): # Only do touch if it does not exist.. else would modify timestamp
            ...
            file_state = self.states / f"{query_id}_{packet_id-1}" # First/initial state
            file_state.touch()

            # Now create commit one
            file.touch()
        # tmp_file = self._packet_file(self.not_finished, query_id, packet_id)
        # with open_file(tmp_file, "w") as f:
    ...
    # -------------------------------------------------------------
    def push_changes(self, query_id, packet_id, new_state): ## Lets 
