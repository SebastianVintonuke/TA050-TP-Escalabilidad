...
class RabbitQueueMiddleware(MessageMiddlewareQueue):
    def __init__(self, queue_name, host = routing.RABBITMQ_HOST):
        ...
        self.exch_type = ''
        #self.rb_sender = self._create_sender() # Not needed If caller does not send messages... but well...
    	try:
	    	...
	    	raise MessageMiddlewareConnectError(f"RabbitMQ connect failed: {e}")
    ...
    def start_consuming(self, on_message_callback):
    	try:    	
	    	...
	    	raise MessageMiddlewareDisconnectedError(f"RabbitMQ connection error at consume: {e}")
	    except Exception as e:
	    	raise MessageMiddlewareMessageError(f"RabbitMQ message handling error: {e}")
    ...
    def send(self, message_builder):
    	try:
	    	...
	    	raise MessageMiddlewareMessageError(f"Message handling error: {e}")
    ...
    def delete(self):
		try:
			...
			raise MessageMiddlewareDisconnectedError(f"RabbitMQ connection error at delete queue: {e}")
        except Exception as e:
