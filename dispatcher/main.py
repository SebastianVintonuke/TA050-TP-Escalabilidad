...
from src.dispatcher import DispatcherServer
...
def main(config_params) -> None:
    ...
    logging.debug(
        f"action: config | result: success | port: {port} | listen_backlog: {listen_backlog} | logging_level: {logging_level} | node_id: {node_id}"
    )
