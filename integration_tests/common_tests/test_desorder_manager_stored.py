...
FIELD_EXP_NEXT = "next_exp"
FIELD_MISSING = "missing"
class MockDesorderStateManager(JSONStateManager):
    def __init__(self, acks = []):
        pass

    def apply_changes(self, state, changes):
        state.expected_next_packet = changes[FIELD_EXP_NEXT]
        state.missing_packets = set(changes[FIELD_MISSING])

        return state

    def serialize_state(self, state):
        return super().serialize_state({
                FIELD_EXP_NEXT: state.expected_next_packet,
                FIELD_MISSING: list(state.missing_packets),
            })
    def serialize_changes(self, changes):
        return super().serialize_changes({
                FIELD_EXP_NEXT: changes.expected_next_packet,
                FIELD_MISSING: list(changes.missing_packets),
            })

    def deserialize_state(self, state_bytes):
        state = super().deserialize_state(state_bytes)

        res= PacketIDTracker()
        res.expected_next_packet = state.get(FIELD_EXP_NEXT, 0)
        res.missing_packets = set(state.get(FIELD_MISSING, []) )

        return res


    def to_deserialized_changes(self, changes):
        return {
                FIELD_EXP_NEXT: changes.expected_next_packet,
                FIELD_MISSING: list(changes.missing_packets),
        }

class TestDesorderManagerStateStorage(unittest.TestCase):
    ...
    def test_simple_query_state_save_changes(self):
        state_manager= MockDesorderStateManager()
        state_storage = q_state.QueryStateStorage("initial_folder", state_manager)

        # Arrange
        changes = PacketIDTracker()

        changes.check_new_packet(2)
    ...
    def test_simple_query_state_push_changes_from_state(self):
        ...
        new_state_res = state_storage.get_new_state("q1",1, state_manager.to_deserialized_changes(changes), 1) # Use ver 0 as base and calc from 'disk'/backedup
        ...
        states = state_storage.load_states()

        self.assertIn("q1", states)

        q1_version, q1_state= states["q1"]

        self.assertEqual(q1_version, 1)
        self.assertEqual(q1_state.expected_next_packet, changes.expected_next_packet)
        self.assertEqual(q1_state.missing_packets, changes.missing_packets)
