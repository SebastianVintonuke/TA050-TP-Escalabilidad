import unittest
from common.state_storage import ack_batched_storage as q_state
...
class BatchedMockStateManager(BaseStateManager):
    def __init__(self, acks = []):
        self.batched_acks = acks
        self.ind_acked = 0
    ...
    def deserialize_changes(self, changes_bytes):
        if self.ind_acked < len(self.batched_acks):
            ack_batch = self.batched_acks[self.ind_acked]
            self.ind_acked +=1
            return changes_bytes.decode(), ack_batch
        return changes_bytes.decode(), [NOT_REGISTERED_PACKET]

class TestBatchedStateStorage(unittest.TestCase):
    ...
    def test_simple_query_state_storage_creates_dirs(self):
        state_storage = q_state.QueryStateStorage("initial_folder", BatchedMockStateManager())
    ...
    def test_simple_query_state_save_changes(self):
        state_manager= BatchedMockStateManager()
    ...
    def test_simple_query_state_register_packet_creates_files_for_query(self):
        ...
        self.assertEqual(file_stat.count_written_times, 1) # it does in fact write inital state
    ...
    def test_simple_query_state_push_changes(self):
        exp_acks = ["msg1"]
        state_manager= BatchedMockStateManager(acks = [exp_acks])
        state_storage = q_state.QueryStateStorage("initial_folder", state_manager)
        ...
        exp_file2 = "initial_folder/packets/finished/q1_1" # Ack file here is one file per query id and packet number... lines are the acks
        ...
        self.assertIn(temp_file, self.mock_fs.deleted_paths)
        deleted_files = self.mock_fs.deleted_paths[temp_file]
        self.assertEqual(len(deleted_files), 2) # Deletes it twice.. once for when loading new state. Then one for acks! 
    ...
    def test_simple_query_state_check_integrity_apply_commited_then_ack_packet(self):
        exp_acks = ["msg1"]
        state_manager= BatchedMockStateManager(acks = [exp_acks])
        state_storage = q_state.QueryStateStorage("initial_folder", state_manager)
    ...
    def test_simple_query_state_check_integrity_discard_apply_not_commited(self):
        state_manager= BatchedMockStateManager(acks = ["msg1"])
        state_storage = q_state.QueryStateStorage("initial_folder", state_manager)
