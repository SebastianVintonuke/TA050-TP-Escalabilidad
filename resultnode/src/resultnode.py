...
class ResultNode:
	...
	def handle_result(self, headers, msg):
		...
		if headers.is_error():
			...
			if handler:
				self.out_middle.send(handler.get_abort_msg(user_id))
		...
		if headers.is_eof():
			...
			self.out_middle.send(handler.handle_new_results(msg,  user_id))
		...
		if counter.is_eof():
			...
			self.out_middle.send(handler.get_eof_msg(user_id))
